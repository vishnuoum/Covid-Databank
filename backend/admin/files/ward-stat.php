...
<?php
  ...
  ?>
  <?php
        if(isset($_GET["client"])&&!isset($_GET["print"])){
      ?>
  <div class="d-flex justify-content-center">
      <form action="ward-stat.php" method="get" class="form-inline">
        <select name="ward" id="ward" class="form-control mb-2 mr-sm-2">
          <option selected disabled>Select a ward</option>
          <?php
            $ward=1;
            while($ward!=42){
              echo "<option value=\"$ward\">Ward $ward</option>";
              $ward++;
            }
          ?>
        </select>
        <button type="submit" class="btn btn-primary mb-2">Go</button>&nbsp;<button type="button" class="btn btn-primary mb-2" onclick="Android.pdf(window.location.href)">Save</button>
      </form>
    </div>
    <?php
        }
    ?>
  <div class="d-flex justify-content-center headingg">
      <h4>Statistics of Ward <?php
        if(isset($_GET["ward"])){
          echo $_GET["ward"];
        }
      ?></h4>
  ...
  <div class="container">
    ...
    <div class="content">
      <?php
        ...
        if($result1->num_rows>0 && isset($_GET["ward"])){
          $i=1;
          while($q=$result1->fetch_assoc()){
            $sql="Select sum(response) as response,question,date from response where question=".$q["id"]." and member=(Select id from members where ward=".$_GET["ward"].") group by date";
